...
class DynamixelDriver(DynamixelDriverProtocol):
    def __init__(
        ...
        self._lock = Lock()
        self._torque_lock = _torque_lock
        self._read_time_period_s = 0.005  # TODO：high CPU
        ...
        self._torque_enabled = self._torque_lock
        try:
    ...
    def set_torque_lock(self, lock: bool):
        """设置电机物理锁定状态"""
        self._torque_lock = lock
        self.set_torque_mode(lock)  # 锁定=禁用扭矩，解锁=启用扭矩

    def is_torque_locked(self) -> bool:
        """获取当前锁定状态"""
        return self._torque_lock
    ...
    def _read_joint_angles(self):
        ...
        try:
            while not self._stop_thread.is_set():
                time.sleep(self._read_time_period_s)
                with self._lock:
            ...
            import traceback
            traceback.print_exc()
            print(f"read_joint_angles error: {e}")
