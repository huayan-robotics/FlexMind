...
# sys.path.insert(0, "/home/robot/fleximind/venv38/lib/python3.8/site-packages")
# import zmq

from fleximind_remote.gello.robots.robot import Robot
from fleximind_remote.gello.robots.huayan import CPSRobot
    ...
    def serve(self) -> None:
        ...
        while not self._stop_event.is_set():
            try:
                ...
                elif method == "immediate_stop":
                    result = self._robot.immediate_stop()
                elif method == "move_j":
                    result = self._robot.move_j(**args)
                elif method == "read_io":
                    result = self._robot.read_io(**args)
                elif method == "blackout":
                    result = self._robot.blackout()
                elif method == "check_state":
                    result = self._robot.check_state()
                elif method == "re_init":
                    result = self._robot.re_init()
                elif method == "_connect_to_robot":
                    result = self._robot._connect_to_robot()
                elif method == "check_power":
                    result = self._robot.check_power()
                # elif method == "power_off":
                #     result = self._robot.power_off()
                else:
...
class ZMQClientRobot(Robot):
    ...
    def blackout(self) -> bool:
        request = {"method": "blackout"}
        self._socket.send(pickle.dumps(request))
        result = pickle.loads(self._socket.recv())
        return result

    def check_state(self) -> bool:
        request = {"method": "check_state"}
        self._socket.send(pickle.dumps(request))
        result = pickle.loads(self._socket.recv())
        return result

    def re_init(self) -> bool:
        request = {"method": "re_init"}
        self._socket.send(pickle.dumps(request))
        result = pickle.loads(self._socket.recv())
        return result
    ...
    def _connect_to_robot(self) -> bool:
        request = {"method": "_connect_to_robot"}
        self._socket.send(pickle.dumps(request))
        result = pickle.loads(self._socket.recv())
        return result

    def check_power(self) -> bool:
        request = {"method": "check_power"}
        self._socket.send(pickle.dumps(request))
        result = pickle.loads(self._socket.recv())
        return result
